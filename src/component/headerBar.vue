<template>
  <div class="header">
    <el-menu
      class="el-menu-demo"
      mode="horizontal"
      background-color="#545c64"
      text-color="#fff"
      active-text-color="#ffd04b">
      <el-menu-item index="0" class="title"><router-link to="/pages/home">豆瓣</router-link></el-menu-item>
      <el-menu-item index="1"><router-link to="/pages/movie">电影</router-link></el-menu-item>
      <el-menu-item index="2"><router-link to="/pages/broadcast">广播</router-link></el-menu-item>
      <el-menu-item index="3"><router-link to="/pages/book">图书</router-link></el-menu-item>
      <el-menu-item index="4"><router-link to="/pages/group">小组</router-link></el-menu-item>
      <el-menu-item index="5"><router-link to="/pages/home"><i class="el-icon-tickets"></i></router-link></el-menu-item>
    </el-menu>
  </div>
</template>
<style lang="scss" scoped>
  .header {
    .title {
      font-size: 2.5em;
      a {
        font-weight: 800;
        color: #00b600!important;
        display: inline-block;
      }
    }
    .talion {
      font-size: 0;
      background: url(data:image/png;base64,) no-repeat;
      background-size: cover;
      width: 2.4rem;
      height: 1.8rem;
      margin-top: 0.4rem;
    }
    .el-menu-item {
      padding: 0 14px;
    }
    .el-icon-tickets {
      font-weight: 700;
      color: #fff;
    }
  }
</style>
<script>
  export default {
    name: 'header-bar',
    data () {
      return {}
    }
  }
</script>
